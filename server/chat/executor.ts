...
async function execListBots(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
...
async function execSwitchBot(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
  if (!cmd.botKey) {
  ...
  await storage.setActiveBotId(userId, bot.id);
  return {
    ok: true,
...
async function execBotStatus(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
  const bot = await resolveBot(userId, cmd.botKey);
  if (!bot) {
...
async function execRunNow(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
...
async function execPauseBot(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
  const bot = await resolveBot(userId, cmd.botKey);
  if (!bot) {
...
async function execResumeBot(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
  const bot = await resolveBot(userId, cmd.botKey);
  if (!bot) {
    return { ok: false, assistantMessage: cmd.botKey ? `'${cmd.botKey}' 봇을 찾을 수 없습니다.` : "활성 봇이 없습니다.", executed: cmd, result: null };
  }
...
async function execAddSource(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
...
async function execRemoveSource(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
...
export async function executeCommand(userId: string, cmd: ParsedCommand): Promise<ExecutionResult> {
  switch (cmd.type) {
    case "list_bots":
      return execListBots(userId, cmd);
    case "switch_bot":
      return execSwitchBot(userId, cmd);
    case "bot_status":
